...
def getPeaks(colPixAve: []):
    ...
    col_diff = signal.savgol_filter(col_diff, 41, 1, mode='nearest')
    tempList = []
...
def getROI(srcImg):
    ...
    #cv.imshow("dst", dst)
    ret, binaryImg = cv.threshold(dst, 0, 255, cv.THRESH_BINARY)
...
def processImgChannel(grayImg, text="", color=(255, 255, 255)):
    ...
    for peak in peaks:
        rows, cols = grayImg.shape
        i = peak.index - 3
        baseVal = 0.5 * (colPixAve[peak.left] + colPixAve[peak.right])
        val = 0
        #for row in range(rows):
        for col in range(peak.left, peak.right):
            val = val + (colPixAve[col] - baseVal)*rows
            #peak.sumPix += grayImg[row, col]
        ave = val/(peak.right - peak.left)
        #peak.avePix = int(peak.sumPix / peak.width)
        peak.sumPix = int(val)
        peak.avePix = int(ave)
...
if __name__ == "__main__":
    filename = "/home/tao/Desktop/IMG_VISION/2023_10_28_18_11_59_.jpg"
    srcImg = cv.imread(filename, 1)
    #$srcImg = cv.resize(srcImg, (240,320))
