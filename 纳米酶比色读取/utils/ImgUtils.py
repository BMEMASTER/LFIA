...
def drawLine(peaks: [], colPixAve):
    """ 绘制曲线图 """
    filterVal = signal.savgol_filter(colPixAve, 15, 1, mode='nearest')
    # 一阶导数
...
def processImgChannel(grayImg, text="", color=(255, 255, 255)):
    ...
    for i in range(len(peaks)):
        ...
        cv.line(projectionImg, lPoint, rPoint, (0, 0, 255), 1)
        # 绘制编号
...
if __name__ == "__main__":
    filename = r"D:\Taoe\Image\dianzhen_linmingdu\1.tif"
    srcImg = cv.imread(filename, 1)
    #srcImg = cv.resize(srcImg, (600, 100))
    cv.imshow("src", srcImg)
    #srcImg = clahe(srcImg)
    # kernel = (5, 5)
    # srcImg = cv.GaussianBlur(srcImg, kernel, sigmaX=15)
    #cv.imshow("After enhancement", srcImg)
    #cv.imshow("Gaussian", srcImg)
    # rows, cols, chs = srcImg.shape
    # # 分离通道
    # chB, chG, chR = cv.split(srcImg)
    gray = cv.cvtColor(srcImg, cv.COLOR_BGR2GRAY)
    #srcImg = cv.resize(srcImg, (1000, 150), interpolation=cv.INTER_LINEAR)
    white_img = np.full(gray.shape, 255, dtype=np.uint8)
    srcImg = white_img - gray
    #cv.imshow("bise", srcImg)
    # # 创建曲线图
    start = time.time()
    peaksR, projectionImgR = processImgChannel(srcImg, "W", (255, 255, 255))
    end = time.time()
    duration = end - start
    print(f"函数运行时长: {duration}秒")
    #peaksR, projectionImgR = processImgChannel(junheng, "G", (0, 255, 0))
    cv.imshow("project", projectionImgR)
    for peak in peaksR:
        print(peak.avePix)
    ...
    #cv.imwrite("C:/Users/Shinelon/Desktop/result_r.png", projectionImgR)
